...
- Name: edge-cases.spec.ts >> Page responsiveness (not hung) >> general chat doesn't freeze the page
- Location: e2e/edge-cases.spec.ts:87:7

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:3000/demo/general
Call log:
  - navigating to "http://localhost:3000/demo/general", waiting until "load"
...
> 88  |     await page.goto("/demo/general");
  ...
  163 |     await textarea.press("Enter");
  164 |     await page.waitForTimeout(1500);
  165 | 
  166 |     await textarea.fill("Tabla de inventario con 10 productos");
  167 |     await textarea.press("Enter");
  168 |     await page.waitForTimeout(1500);
  169 | 
  170 |     await textarea.fill("Gráfica de usuarios por región");
  171 |     await textarea.press("Enter");
  172 |     await page.waitForTimeout(3000);
  173 | 
  174 |     // PROBE: Toggle must still work after multiple requests
  175 |     const toggle = page.locator('button[title="Modo claro"], button[title="Modo oscuro"]').first();
  176 |     await expect(toggle).toBeVisible({ timeout: 5000 });
  177 | 
  178 |     const isDarkBefore = await page.evaluate(() =>
  179 |       document.documentElement.classList.contains("dark")
  180 |     );
  181 |     await toggle.click();
  182 |     await page.waitForTimeout(500);
  183 | 
  184 |     const isDarkAfter = await page.evaluate(() =>
  185 |       document.documentElement.classList.contains("dark")
  186 |     );
  187 |     expect(isDarkAfter).not.toBe(isDarkBefore);
  188 |   });
```